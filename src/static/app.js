...
const nnData =  null //JSON.parse(nnBoundary)
var nnCounter = 0
var intervalID
...
let body = {
    'TrainPosSlider' : d3.select('#TrainPosRange').node().value,
    'TestPosSlider' : d3.select('#TestPosRange').node().value,
    'TrainNegSlider' : d3.select('#TrainNegRange').node().value,
    'TestNegSlider' : d3.select('#TestNegRange').node().value,
    'covTrue' : covTrue,
    'covFalse' : covFalse,
    'muTrue' : muTrue,
    'muFalse' : muFalse
...
async function getData(content){
    ...
    const response = await fetch('http://192.168.46.21:8000/RandomData/generateData/', {
        method: "POST",
        mode: "cors",
        headers: {
...
async function getModelResults(content){
    ...
    const response = await fetch('http://192.168.46.21:8000/trainModel/train/', {
        method: "POST",
        mode: "cors",
        headers: {
          "Content-Type": "application/json",
...
positiveCovMatrix0.addEventListener("change", (event) => {
    ...
    if (parseFloat(positiveCovMatrix0.value) < 0){
        positiveCovMatrix0.value = 0
    }
});
...
positiveCovMatrix3.addEventListener("change", (event) => {
    ...
    if (parseFloat(positiveCovMatrix3.value) < 0){
        positiveCovMatrix3.value = 0
    }
});
...
negativeCovMatrix0.addEventListener("change", (event) => {
    ...
    if (parseFloat(negativeCovMatrix0.value) < 0){
        negativeCovMatrix0.value = 0
    }
});
...
negativeCovMatrix3.addEventListener("change", (event) => {
    ...
    if (parseFloat(negativeCovMatrix3.value) < 0){
        negativeCovMatrix3.value = 0
    }
});

negativeMu0.addEventListener("change", (event) => {
...
const TrainPosRange = document.querySelector("#TrainPosRange");
const TrainNegRange = document.querySelector("#TrainNegRange");
const TestPosRange = document.querySelector("#TestPosRange");
const TestNegRange = document.querySelector("#TestNegRange");

const TrainPosSlider = document.querySelector("#TrainPosSlider");
const TrainNegSlider = document.querySelector("#TrainNegSlider");
const TestPosSlider = document.querySelector("#TestPosSlider");
const TestNegSlider = document.querySelector("#TestNegSlider");


TrainPosRange.addEventListener("change", (event) => {
    val = Math.floor(parseFloat(TrainPosRange.value))

    val = Math.max(Math.min(val, 1000), 5)

    TrainPosRange.value = val
    TrainPosSlider.value = val

});

TrainNegRange.addEventListener("change", (event) => {
    val = Math.floor(parseFloat(TrainNegRange.value))

    val = Math.max(Math.min(val, 1000), 5)

    TrainNegRange.value = val
    TrainNegSlider.value = val

});

TestPosRange.addEventListener("change", (event) => {
    val = Math.floor(parseFloat(TestPosRange.value))

    val = Math.max(Math.min(val, 1000), 5)

    TestPosRange.value = val
    TestPosSlider.value = val

});

TestNegRange.addEventListener("change", (event) => {
    val = Math.floor(parseFloat(TestNegRange.value))

    val = Math.max(Math.min(val, 1000), 5)

    TestNegRange.value = val
    TestNegSlider.value = val
